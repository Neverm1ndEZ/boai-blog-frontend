...
export default function BlogCards({ blogs }: { blogs: any }) {
	return (
		<main className="grid grid-cols-1 md:grid-cols-2 gap-10 p-10 md:border-r-2 border-gray-600 h-full">
			{blogs?.data?.map((blog: any) => (
				<div key={blog.id} className="relative">
					<Link href={`/blog/${blog.attributes.slug}`} className="">
						<div className="min-h-[31rem] bg-[#242424] p-4 rounded-xl shadow-[0_0_15px_orange] hover:scale-105 transform ease-in-out duration-300">
							...
							<div className="grid justify-between my-5 space-y-3 ">
